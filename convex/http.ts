...
http.route({
  path: "/clerk-webhook", //from clerkdashboard url we create for webhooks
  method: "POST",
  handler: httpAction(async (ctx, request) => {
    const webhookSecret = process.env.CLERK_WEBHOOK_SECRET; //that we saved in clerk dashboard as Environment Variables
    if (!webhookSecret) {
...
// from convex dashboard -> env variables
